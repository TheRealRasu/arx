...
public class KAPDisplay {
	...
	public void displayData (final StatisticsSummary statSum, final String attribute, final DataHandle dataHandle){
		...
		if(statSum.getScale()==ScaleOfMeasure.NOMINAL){
			...
			+statSum.getMaxAsString()+"\nMinimum:\t\t"+statSum.getMinAsString());
				...
				medianMinMaxChart.getAxisSet().getXAxis(0).getTitle().setVisible(false);
				medianMinMaxChart.getAxisSet().getYAxis(0).getTitle().setVisible(false);
				
				
				medianMinMaxChart.getTitle().setText("Displaying the Median, Minimum and Maximum of the attribute "+attribute);
				...
				IBarSeries barSeries = (IBarSeries) medianMinMaxChart.getSeriesSet()
		...
		} else if (statSum.getScale()==ScaleOfMeasure.INTERVAL||statSum.getScale()==ScaleOfMeasure.RATIO){
		...
		//Point p = textLabel.computeSize(SWT.DEFAULT, SWT.DEFAULT);
        //textLabel.setBounds(50, 50, p.x+5, p.y+5);
	...
	public void intervalRatioText(Display display, StatisticsSummary statSum, String attribute){
		
		
		final Shell modeShell=new Shell(display, SWT.CLOSE);
		modeShell.setText("Displaying the values of the attribute "+attribute);
		...
		String displayText=("Attribute:\t\t"+attribute+"\nScale Of Measure:\t\t"
				...
				+"\nMinimum:\t\t"+statSum.getMinAsString()
				+"\nArithmetic Mean:\t\t"+statSum.getArithmeticMeanAsString()
				+"\nSample Variance:\t\t"+statSum.getSampleVarianceAsString()
				+"\nPopulation Variance:\t"+statSum.getPopulationVarianceAsString()
		...
		if(statSum.isGeometricMeanAvailable()){
			displayText=displayText.concat("\nGeometric Mean:\t\t"+statSum.getGeometricMeanAsString());
		}
	...
	public void intervalRatioBarSeries(Display display, StatisticsSummary statSum, String attribute){
		
		
		final Shell intervalRatioShell=new Shell(display, SWT.CLOSE);
		intervalRatioShell.setSize(800, 600);
		intervalRatioShell.setText("Displaying the Median, Minimum and Maximum of the attribute "+attribute);
		intervalRatioShell.setLayout(new FillLayout());

		
		Chart intervalRatioChart=new Chart(intervalRatioShell,SWT.NONE);

		intervalRatioChart.getAxisSet().getXAxis(0).getTitle().setVisible(false);
		intervalRatioChart.getAxisSet().getYAxis(0).getTitle().setVisible(false);
		IAxisTick xAxisTick= intervalRatioChart.getAxisSet().getXAxis(0).getTick();
		
		
		intervalRatioChart.getTitle().setText("Displaying the Median, Minimum and Maximum of the attribute "+attribute);
